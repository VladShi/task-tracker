...
- Поддерживает операции с задачами через REST API:
    - `POST /tasks` — создание задачи.
    - `GET /tasks` — получение списка задач.
    - `PUT /tasks/{taskId}` — редактирование задачи.
    - `DELETE /tasks/{taskId}` — удаление задачи.
    - `GET /tasks/{taskId}` — получение задачи по ID.
- Nginx в `frontend` модуле для проксирования запросов на бэкенд использует префикс `/api/`, например (`/api/tasks`).
...
- POST-запрос на `/auth/login` для аутентификации.

### Страница регистрации

![register](assets/register.png)

URL - '/auth/register' (обрабатывается через модальное окно на главной странице)
- POST-запрос на `/user` для регистрации нового пользователя.
